 ...
 *
 * $Id: DatabaseConnection.php 2679 2013-12-19 15:37:17Z fholzinger $
 *
...
class DatabaseConnection extends \TYPO3\CMS\Core\Database\DatabaseConnection {
	 ...
	 * @return boolean|\mysqli_result|object MySQLi result object / DBAL object
	 */
	public function exec_SELECTquery ($select_fields, $from_table, $where_clause, $groupBy = '', $orderBy = '', $limit = '', $dbgModes = 0) {
		if (!$this->isConnected) {
			$this->connectDB();
		}
		...
		if ($tablePart != '') {
			...
			if (!in_array($lowerTable, $keyWords)) {
				if (
					...
					isset($GLOBALS['TCA'][$lowerTable]) && $this->dbgTca ||
					$this->dbgTable[$lowerTable]
				) { // is this a table name inside of TYPO3?
					$bEnable = TRUE;
				} else if (!$bErrorCase && $this->dbgTca) { // an error message is also shown if the $TCA is not loaded in the FE
					$bDisable = TRUE;
				}
			}
		}
	...
	public function getLastInsertId ($table) {
		...
		if (
			...
			$rowArray = $this->exec_SELECTgetRows('uid', $table, 'uid=LAST_INSERT_ID()', '', '', '1');

			if (isset($rowArray) && is_array($rowArray)) {
				$result = $rowArray['0']['uid'];
			} else {
				$rowArray = $this->exec_SELECTgetRows('id', $table, 'id=LAST_INSERT_ID()', '', '', '1');
				if (isset($rowArray) && is_array($rowArray)) {
					$result = $rowArray['0']['id'];
				}
			}
		}
		...
		if ($error) {
			if (!intval($this->dbgConf['DISABLE_ERRORS'])) {
				...
				if (
					...
					if ($this->dbgOutput == 'error_log' || $this->dbgTextformat) {
			...
			if ($this->dbgQuery[$mode] &&
				!$bDisable &&
				($this->dbgTable['all'] || $bEnable || !$table) &&
				(count($this->dbgFeUser) == 0 || $this->dbgFeUser[$feUid . '.']) &&
